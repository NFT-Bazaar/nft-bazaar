...
function PanelSearch({ isOpen }: MyProps) {
  ...
  return (
    ...
    >
        ...
        <div key={index} className="mb-4">
          <div className="flex items-center">
            ...
            <div className="text-sm ml-3">
              <label htmlFor={item.name} className="font-medium text-gray-500">
                {item.title}
              </label>
            </div>
          </div>
      ...
      <button className="w-full bg-gray-800 text-white text-sm font-bold py-2 px-4 rounded-md transition duration-300">
        Search
      </button>
    </div>
  );
